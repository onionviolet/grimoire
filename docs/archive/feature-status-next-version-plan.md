> **Retired 2026-09-01.** The v1.25-era forward plan that used to sit at the
> foot of `docs/feature-status.md`. Its slices were delivered, superseded, or
> reduced to the handful of items now in `.planning/BACKLOG.md`. A status doc
> should not also be a plan; this is why it was split out. Kept for the
> sequencing reasoning, not for its status claims.

# Next-version implementation plan (archived)
