...
export default function HeroDetailFrame<Id extends string>({
  ...
  // What is on the stage, as a typed descriptor rather than as an inference
  // from whether a prop happened to be set. See `lib/heroStage.ts`.
  const plate = resolveHeroPlate(backdropImage, renderSrc);
  const plateComposition = plate ? heroPlateComposition(plate) : null;

  return (
    <div
      className="relative flex h-full flex-col overflow-y-auto lg:flex-row lg:overflow-hidden"
      style={heroStageVars()}
    >
      {/* The plate: full-bleed behind every panel so it can bleed through the
      ...
      {/* The veil: a progressive frosted-glass stack, every layer masked with a
          long smooth taper so nothing has a hard right edge. The geometry lives
          in `lib/heroStage.ts` because the clear zone is derived from the
          heaviest layer's transparent stop, and a stop written in two places
          would let the two disagree. */}
      <div
        aria-hidden
        className={`pointer-events-none absolute inset-y-0 left-0 z-[5] hidden lg:block ${HERO_STAGE_VEIL.widthClass}`}
      >
        {HERO_STAGE_VEIL.layers.map((layer) => (
          <div key={layer.blurPx} className="absolute inset-0" style={veilLayerStyle(layer)} />
        ))}
        <div className="absolute inset-0" style={{ background: HERO_STAGE_VEIL.tint }} />
      </div>
