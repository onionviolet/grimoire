...
describe('RadialWheelPreview dressing', () => {
  ...
  async function render(dressing: Dressing, withProp = true): Promise<HTMLDivElement> {
    ...
    await act(async () => {
      root.render(
        withProp ? (
          <RadialWheelPreview menuIndex={0} menuName="Go" icon="quick" items={ITEMS} focusedSlot={1} onSelectSlot={() => {}} dressing={dressing} />
        ) : (
          <RadialWheelPreview menuIndex={0} menuName="Go" icon="quick" items={ITEMS} focusedSlot={1} onSelectSlot={() => {}} />
        )
      );
    });
    return host;
