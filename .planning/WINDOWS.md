...
| 6 | 11 | failing-test | electron/main/services/forgeBridge.test.ts | 535 | 'serves more concurrent connections than a browser will open' times out on this macOS machine (Node 26.4) only: of 8 concurrent loopback connects, 6 reach the server's connection event and 2 never arrive, while a plain node:http server on the same port 43110 answers a 40-connect burst in 22 ms both bare and under vitest. The bridge file is unchanged since v1.27.1 and the case passed on 2026-09-01, so it is environmental and unexplained, not caused by phases 10 or 11. Left red rather than quarantined: it is a security regression test and CI on Node 20 is the authoritative run. | open |  | 2026-09-05T00:00:00.000Z |  |
