# Requirements Archive: v1.27 verified, supported, coherent
...
- [x] **REQ-global-inventory-coherence**: Global reads as one inventory. It may use an `All content | Visuals | Sounds` filter, but it must not strand someone in an empty sound category while hiding installed global content. Open layout defects re-confirmed live after the classification fix: empty categories still render (`Announcer 0`, `Ambience 0`), the Visuals rail still shows sound-shaped buckets, the header reports `20 mods` on Visuals against `15 mods` on Sounds for one inventory, and `Pak92`/`Pak93` are unusable as list entries. The source is decision-gated and says "do not merge the surfaces yet": compare the existing flows and prototype one shared shell before moving data or deleting a route. **Competing variant with REQ-sound-locker-surface.** **Amended 2026-08-09 (plan 05-04):** all four originally-diagnosed layout defects now read as closed. The header count disagreement was already closed structurally before this phase by the shared `countGlobalInventoryMods` denominator. Empty categories no longer render, reversing the deliberate as-shipped decision per D-04. The tri-state view arrived as the widened `LockerMode` in plan 05-01. The unusable bare pak list entries are closed by this plan's derived description: a Global card whose name is only a pak slot leads with what the mod writes (`derivePakDescription` over its own VPK entries), keeps the raw pak name as a secondary identity line, and labels unreadable or empty paks as unknown rather than guessing (D-19).
- [x] **REQ-sound-locker-surface**: A Locker-style home for installed sound mods at its own route (`/locker/sounds` with `/locker/sounds/hero/:hero` and `/locker/sounds/global`), on the reasoning that global sound content has no home at all. **Competing variant with REQ-global-inventory-coherence, and the one place the record is not neutral:** this route was built and then folded back into the Locker shell, because two hero grids for one hero's content was the distance the lane set out to remove. `SoundLocker.tsx` is now `GlobalSoundShelf` inside the Global drill-in and the legacy URLs are rewritten by `legacySoundTarget`. The requirement is kept open because whether the route should survive is exactly the contested question, not because the route is missing. Its inventory model, rows, audition, and exact-entry inspection are delivered and are not in dispute. **Amended 2026-08-09 (plan 05-04):** the contested variant is resolved by D-01 in favour of the Locker Global drill-in as the single canonical home, so the dedicated route does not return. D-02 keeps the folded-back variant's useful capabilities inside that shell: exact-path ownership, runtime active source, provenance, audition, enable and disable, conflict disclosure, and the Foundry cross-link, carried by `src/components/locker/GlobalSoundShelf.tsx`. The shipped legacy `/locker/sounds*` URLs continue to resolve through `legacySoundTarget` and `resolveLockerRoute` in `src/lib/lockerMode.ts`, asserted in `src/lib/lockerMode.test.ts` including after plan 05-01 widened the default section to All content. The deferred idea stays deferred: a separate top-level route is reconsidered only if the shared Global and hero Sounds surfaces cannot expose a concrete required workflow without duplication.
...
- [x] **REQ-portrait-alias-sweep**: The one remaining path to a root cause on issue #4 ("Abrams Portraits resolves to no families"), written while the defect is not reproducing. The three-way test holds two facts side by side per hero: what `resolvePortraitHero` and `portraitCodenamesForHero` return, and which codenames the loaded catalog actually contains. Verdicts: in-catalog and unresolved is an alias miss; not-in-catalog and resolved means the empty state is honest; resolved with zero families means `portraitFamilyKey` is wrong; neither means the hero is absent from this build. Leg A (alias-table self-consistency) is safe to run alongside other work; Legs B and C need a committed working tree because the dev-driver drives the working tree. **Amended 2026-08-09 (plan 05-05):** all three legs have run. Leg A lives in `src/lib/heroPortraitIdentity.test.ts`; Legs B and C ran against the loaded build on dev slot 2, and `docs/portrait-alias-sweep-plan.md` is the verdict record rather than a restatement here. Outcome: no alias miss was found among the fifteen mismatch heroes, every one renders portrait families on the scoped Foundry surface, and the Abrams "resolves to no families" defect did not reproduce on this build.
...
| REQ-global-inventory-coherence | Phase 5 | Complete |
| REQ-sound-locker-surface | Phase 5 | Complete as amended (D-01, D-02) |
| REQ-portrait-journey-consolidation-gated | Phase 5 | Complete |
| REQ-portrait-alias-sweep | Phase 5 | Complete |
